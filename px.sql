REM
REM Script to monitor parallel queries (Doc ID 457857.1)
REM
...
px.degree "Actual DOP", s.sql_id
--se when px.qcinst_id is NULL then 'alter system kill session '''||px.sid||','||px.serial#||',@'||px.inst_id||''';' else ' ' end killcmd
...
px.req_degree "Req. DOP",
px.degree "Actual DOP", s.sql_id,
case when px.qcinst_id is NULL then 'alter system kill session '''||px.sid||','||px.serial#||',@'||px.inst_id||''';' else ' ' end killcmd
from gv$px_session px,
gv$session s ,
gv$px_process pp,
gv$session_wait sw
...
column target format a32;
column units format a12;
