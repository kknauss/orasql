...
column "Alloc(MB)" format 9999990.99 ;
column "Alloc(GB)" format  999990.99 ;
column "Free(MB)"  format 9999990.99 ;
...
--            (nvl(sum_free_blks,0)*(F.block_size/1024))                                 "Free(KB)",
       ...
       round( (sum_alloc_blks*(F.block_size/(1024*1024*1024))) ,2)  - round( (nvl(sum_free_blks,0)*(F.block_size/(1024*1024*1024)))  ,2)                "Used(GB)",
...
--So we can calculate freespace for any tablespaces that are not fully used........................................
	...
	from
		v$tablespace a, 
		(select ts#, sum(blocks) blocks from v$tempfile group by ts#) b, 
		(select tablespace_name, sum(blocks_used) blocks_used from v$temp_extent_pool group by tablespace_name) c
	where a.name = c.tablespace_name(+)
	and a.ts#  = b.ts# 
	and a.name not in (select distinct tablespace_name from dba_free_space)) C,
...
ORDER BY A.tablespace_name 
	--nvl(sum_free_blks,0)/sum_alloc_blks
;
