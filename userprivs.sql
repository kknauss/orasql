...
SELECT GRANTEE, account_status, expiry_date, lock_date, LPAD(' ',(DEPTH-1)*5)||PRIVILEGE AS PRIVILEGE, PRIVTYPE , ADMIN_OPTION, IS_DEFAULT, IS_GRANTABLE
FROM (
            WITH X AS
            (
                ...
                WHERE 1=1
                --AND GRANTEE NOT IN ('BRAVURA_SELECT')

                --AND PRIVILEGE <> 'SELECT'
            )
            SELECT GRANTEE, PRIVTYPE, PRIVILEGE, ADMIN_OPTION, IS_DEFAULT, IS_GRANTABLE, LEVEL AS DEPTH
            FROM X
            START WITH GRANTEE = upper('&usernm')
            CONNECT BY PRIOR PRIVILEGE = GRANTEE
            ORDER SIBLINGS BY PRIVILEGE
) Y, DBA_USERS Z
WHERE Y.GRANTEE = Z.USERNAME(+);
      
--SECMKTG_SELECT,ATIGAD_DBA
--LOS_SELECT
